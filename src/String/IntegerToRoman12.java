 ...
 *         Example 5:
 * 
 *         Input: 1994 Output: "MCMXCIV" Explanation: M = 1000, CM = 900, XC =
 *         90 and IV = 4.
 */
public class IntegerToRoman12 {
	...
	public String intToRoman(int num) {
		if (num == 0)
			return "";
		// the range of number is between 0 to 3999
		String M[] = { "", "M", "MM", "MMM" };
	...
	public String intToRoman2(int num) {
		...
		String[] carr = new String[] { "I", "IV", "V", "IX", "X", "XL", "L", "XC", "C", "CD", "D", "CM", "M" };
		StringBuilder res = new StringBuilder();
		int maxInd = arr.length - 1;
		while (num > 0) {
			while (num < arr[maxInd]) {
				maxInd--;
			}
			res.append(carr[maxInd]);
			num -= arr[maxInd];
		}
		return res.toString();
	}
